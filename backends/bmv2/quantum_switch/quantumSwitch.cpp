...
/**
 * This file implements the quantum switch model
 */
...
#include "quantumSwitch.h"
#include "backends/bmv2/quantum_switch/options.h"
...
void ParseV1QArchitecture::modelError(const char* format, const IR::Node* node) {
    ::error(ErrorType::ERR_MODEL,
            (cstring(format) + "\nAre you using an up-to-date v1quantum.p4?").c_str(), node);
}

bool ParseV1QArchitecture::preorder(const IR::PackageBlock* main) {
    auto parser = main->findParameterValue(structure->parser_name);
    ...
    auto verify = main->findParameterValue(structure->verify_checksum_name);
    ...
    auto ingress = main->findParameterValue(structure->ingress_name);
    ...
    auto compute = main->findParameterValue(structure->compute_checksum_name);
    ...
    auto deparser = main->findParameterValue(structure->deparser_name);
    ...
    structure->non_pipeline_controls.emplace(
        deparser->to<IR::ControlBlock>()->container->name);
...
void
QuantumSwitchBackend::modelError(const char* format, const IR::Node* node) const {
    ::error(ErrorType::ERR_MODEL,
            (cstring("%1%") + format +
             "\nAre you using an up-to-date v1quantum.p4?").c_str(), node);
}
...
cstring
QuantumSwitchBackend::createCalculation(cstring algo, const IR::Expression* fields,
...
void
QuantumSwitchBackend::convertChecksum(const IR::BlockStatement *block, Util::JsonArray* checksums,
...
void QuantumSwitchBackend::createActions(ConversionContext* ctxt, V1QProgramStructure* structure) {
...
void
QuantumSwitchBackend::convert(const IR::ToplevelBlock* tlb) {
    structure = new V1QProgramStructure();

    auto parseV1Arch = new ParseV1QArchitecture(structure);
    auto main = tlb->getMain();
    if (!main) return;  // no main

    if (main->type->name != "V1Quantum")
        ::warning(ErrorType::WARN_INVALID, "%1%: the main package should be called V1Quantum"
    ...
    // as a "flag" to indicate that a certain quantum_switch primitive has been
    ...
    conv = new QuantumSwitchExpressionConverter(refMap, typeMap, structure, scalarsName);

    auto ctxt = new ConversionContext(refMap, typeMap, toplevel, structure, conv, json);

    auto hconv = new V1QHeaderConverter(ctxt, scalarsName);
    program->apply(*hconv);

    auto pconv = new ParserConverter(ctxt, "parser");
    structure->parser->apply(*pconv);

    createActions(ctxt, structure);

    auto cconv = new ControlConverter<Standard::Arch::V1MODEL>(
        ctxt, "ingress", options.emitExterns);
    ...
    auto dconv = new DeparserConverter(ctxt, "deparser");
